...
import {sourceAcks,sourceArgs} from '../src/source-scans.js';
...
test('Actual stdio -> HTTP: both URL styles, bound client identity and no uncertain auto-retry',async()=>{
  ...
  const server=createServer(async(req,res)=>{
    ...
    if(path==='/tamrank/v2/scans/sources/capabilities')return send({schema_source_jobs:support});
    if(path==='/tamrank/v2/scans/maintenance/capabilities'){res.statusCode=403;return send({code:'scope'});}
    if(path==='/tamrank/v2/scans/sources/preview'){assert.equal(req.method,'GET');assert.equal(u.searchParams.get('post_id'),'123');return send({source:{revision:hash}});}
    ...
    if(path==='/tamrank/v2/scans/sources/proposals'){assert.deepEqual(value,sourceArgs(plan));return send({plan:{job_id:id},plan_hash:hash});}
    assert.equal(path,'/tamrank/v2/scans/sources/'+id+'/run');assert.deepEqual(value,sourceArgs(run));
  ...
  try{for(const style of ['pretty','query']){
    ...
    try{
      ...
      for(const a of [preview,plan])assert.ok(!(await client.callTool({name:'start_scan',arguments:a})).isError);
      const before=requests.length;const uncertain=await client.callTool({name:'start_scan',arguments:run});
      ...
      assert.ok(!(await client.callTool({name:'start_scan',arguments:run})).isError,'Explicit exact replay returns stored receipt');
    }finally{await client.close();}
  }assert.equal(executions,2);}finally{await new Promise(r=>server.close(r));}
});
