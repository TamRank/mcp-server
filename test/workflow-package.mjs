...
assert.ok(process.argv.slice(2).every(arg=>['--allow-network','--native-reads','--native-scans','--native-scan-receipts','--native-fields','--native-redirects','--native-schema','--native-beta-upgrade','--native-beta-reads','--native-beta-redirects'].includes(arg)),'Unknown installation-test argument');
if(nativeBetaUpgrade||nativeBetaReads||nativeBetaRedirects)assert.ok(process.env.TAMRANK_TEST_BETA_ZIP,'Exact distributed beta path required');
let nativePro;
...
try {
  ...
  if(nativeCases.length){
    ...
    for(const [mode,expected] of nativeCases){
      const reads=mode==='stored-reads'||mode==='stored-reads-multisite',scans=mode==='pagespeed-scans'||mode==='pagespeed-receipts',beta=['beta-upgrade','beta-reads','beta-redirects'].includes(mode);
      console.log('RUN INSTALLED: '+mode+' → extracted entry → '+(reads||scans?'owned HTTP':'verified TLS')+' → owned WordPress.');
      const nativeArgs=beta?[join(nativePro,'docs/mcp-phase4e-beta-upgrade-wordpress.mjs'),'--installed-mcp',...(mode==='beta-reads'?['--stored-reads']:mode==='beta-redirects'?['--redirects']:[])]:scans?[join(root,'test/workflow-pagespeed-native.mjs'),...(mode==='pagespeed-receipts'?['--result-journal']:[])]:reads?[join(nativePro,mode==='stored-reads-multisite'?'docs/mcp-phase4e-read-network-wordpress.mjs':'docs/mcp-phase4e-read-wordpress.mjs')]:[join(nativePro,'docs/mcp-phase4c-change-store-wordpress.mjs'),'--'+mode];
