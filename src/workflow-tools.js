...
import {validSchemaExecutionResponse} from './schema-execution.js';
...
export function registerWorkflowTools(server, client, { profile = 'core', preflight = { ok: true }, capabilities = null, maintenanceOnly = false, recovery = null } = {}) {
  ...
  const proposalSchema=schemaStorage?{...fieldProposalSchema,items:z.array(z.union([
  ...
  const register = (name, def, canonical = name, deprecated = false) => {
    ...
    server.registerTool(name, { description: def.description, inputSchema: schema,
      ...
      if(def.fieldExecution||nativePlan||nativeRead){
        ...
        if(nativeRead||rollback?!(fieldAllowed||redirectAllowed||(nativeRead&&schemaExecutionRead)):!(redirectPlan||redirectExecute?redirectAllowed:fieldAllowed))
        ...
        try{
          ...
          const valid=isSchema?nativeRead&&schemaExecutionRead&&validSchemaExecutionResponse(data,id,hash)
