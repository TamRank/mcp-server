...
export function workflowDefinitions() {
  return {
    ...
    diagnose_page: { description: 'Stored facts, PageSpeed lab evidence and query stability. Stability accepts paginated summaries or exact query daily rows. Never starts scans.', schema: { post_id: pageId, section: z.enum(['overview','metadata','gsc','index','pagespeed','stability']).optional(), query: z.string().min(1).max(512).refine(v=>Buffer.byteLength(v,'utf8')<=512 && !/[\x00-\x1f\x7f]/.test(v)).optional(), ...paging }, path: a => `/pages/${a.post_id}/diagnosis`, omit: ['post_id'],
